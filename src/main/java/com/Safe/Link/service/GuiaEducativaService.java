...
import com.Safe.Link.DTO.CompartirGuiaDTO; // US-29
...
public class GuiaEducativaService {
    ...
    // US-29 — Escenarios 1 y 2: generar payload para compartir
    public CompartirGuiaDTO generarEnlaceCompartir(Long id, String baseUrl) {
        GuiaEducativa guia = repository.findById(id)
                .orElseThrow(() -> new RuntimeException("Guía no encontrada: " + id));

        String urlPublica = baseUrl + "/api/educacion/guias/" + guia.getId();

        String textoCompartir = "Mira esta recomendación oficial de prevención en SafeLink: "
                + "\"" + guia.getTitulo() + "\" → " + urlPublica;

        return CompartirGuiaDTO.builder()
                .idGuia(guia.getId())
                .titulo(guia.getTitulo())
